...
document.addEventListener('click', (e) => {
    if (!audioInitialized) { initAudio().then(() => { audioInitialized = true; playDroplet(); }).catch(() => {}); } else playDroplet();
});
...
function createParticles() {
    ...
    for (let i = 0; i < 40; i++) {
        ...
        p.style.cssText = `width:${sz}px; height:${sz}px; left:${Math.random()*100}%; top:${Math.random()*100}%; animation-delay:${Math.random()*10}s; animation-duration:${Math.random()*10+8}s; opacity:${Math.random()*0.4+0.1};`;
